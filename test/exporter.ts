...
describe('Exporter internals', () => {
  ...
  it('Exporter.save builds zip with DB and media', async () => {
    ...
    expect(zipFileSpy).toHaveBeenCalledWith('collection.anki2', expect.any(Buffer));
  ...
  it('Exporter.addCard populates note and card rows', () => {
    ...
    const exporterUpdateSpy = vi.spyOn(exporter as unknown as { _update: Exporter['_update'] }, '_update');

    exporter.addCard(front, back);

    expect(exporterUpdateSpy).toHaveBeenCalledTimes(2);

    const [notesCall, cardsCall] = exporterUpdateSpy.mock.calls as Array<[string, Record<string, string>]>;
  ...
  it('Exporter.addCard handles tag array', () => {
    ...
    const [notesCall] = exporterUpdateSpy.mock.calls as Array<[string, Record<string, string>]>;
    const notesUpdate = notesCall?.[1] as Record<string, string>;
  ...
  it('Exporter.addCard updates duplicates in place', () => {
    ...
    const [firstNotesCall, firstCardsCall, secondNotesCall, secondCardsCall] = exporterUpdateSpy.mock.calls as Array<
