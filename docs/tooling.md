...
`pnpm test`, because they need Python and a wheel off PyPI, which the JavaScript
...
- oxfmt uses its own defaults, `printWidth` 100 and `sortPackageJson`, plus
...
instead, since that is two specifiers for one module.
...
about import _style_ rather than a free tightening. Take it together with
...
| Wrong for this package              | `import/no-nodejs-modules` in a Node library; `import/no-named-export` against a published named export; `new-cap` against the `AnkiExport` factory                                                                                                  |
...
object literal, so `test/deck-round-trip.test.ts` clones them before asserting.
